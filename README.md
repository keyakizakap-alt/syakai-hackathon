...
- **警告フィルタは安全側に倒す**：偽陰性（警告の失効）が実害なので、迷ったら必ず警告する
...
プリセット4例（極性反転 / 衝突回避語の反転 / 警告の越境失効 / 会計規範の非対称）を判定できる。
